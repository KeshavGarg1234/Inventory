...
import { CheckCircle, Circle, Trash2, MoreVertical, QrCode, Info, HandPlatter, UserX, Undo, VenetianMask, User as UserIcon, Phone, Building, Briefcase, Calendar as CalendarIcon, FileDown, BookCopy, UserPlus, History } from 'lucide-react';
import jsPDF from 'jspdf';
import { QRCodeCanvas } from 'qrcode.react';
import { DeleteConfirmationDialog } from './delete-confirmation-dialog';
import { useAuth } from '@/context/auth-context';
import { SelfAllotRequestDialog } from './self-allot-request-dialog';
import { AllotmentHistoryDialog } from './allotment-history-dialog';
...
export function UnitsTable({ item, users, onSubItemDelete, onLotDelete, onAllotRequest, onStatusChangeRequest }: UnitsTableProps) {
    ...
    const handleDownloadLotQRCodes = (lotName: string, subItemsInLot: SubItem[]) => {
      const doc = new jsPDF();
      const qrSize = 25; // Balanced size for logo visibility
      const margin = 10;
      const gutter = 5;
      const qrsPerRow = 6; // Fit more on one page
      ...
      subItemsInLot.forEach((subItem, index) => {
        ...
        if (canvas) {
            ...
            doc.setFontSize(6);
            doc.text(subItem.id, x + qrSize / 2, y + qrSize + 4, { align: 'center' });
        }
      ...
      <>
        ...
        <div ref={qrContainerRef} style={{ position: 'absolute', top: '-9999px', left: '-9999px' }}>
            {(item.subItems || []).map(subItem => (
                <QRCodeCanvas 
                    key={subItem.id} 
                    value={getQrDataForSubItem(subItem)} 
                    size={256} 
                    level={"H"} 
