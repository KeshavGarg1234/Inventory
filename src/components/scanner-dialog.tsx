...
export function ScannerDialog({ open, onOpenChange, fromSecurePage = false, onSecureScanSuccess }: ScannerDialogProps) {
  ...
  const onScanSuccess = async (decodedText: string, result: Html5QrcodeResult) => {
    ...
    if (scannerRef.current && scannerRef.current.getState() === Html5QrcodeScannerState.SCANNING) {
      try {
        ...
        console.warn("Error stopping scanner, may have already been stopped.", e);
      }
    }
  ...
  useEffect(() => {
    if (open) {
      // Use a short delay to allow the dialog to render before starting the scanner
  ...
  }, [open, fromSecurePage, onSecureScanSuccess]);

  // Cleanup on unmount
  useEffect(() => {
    return () => {
      if (scannerRef.current && scannerRef.current.isScanning) {
        scannerRef.current.stop().catch(e => console.warn("Error stopping scanner on unmount", e));
      }
    }
  }, []);
